...
function Detail({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
 ...
 <p className={`transition-colors duration-300 ${mono ? 'text-slate-900 dark:text-white font-mono text-sm bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-transparent px-3 py-2 break-all' : 'text-slate-900 dark:text-white font-medium text-lg'}`}>{value}</p>
 </div>
 );
}
